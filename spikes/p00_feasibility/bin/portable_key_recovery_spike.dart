...
Future<void> main() async {
  ...
  final recoveredWrappingKey = await kdf.deriveKeyFromPassword(
    password: passphrase,
    nonce: recoverySalt,
  );
  final recovered = await cipher.decrypt(
    recoveryBox,
    secretKey: recoveredWrappingKey,
  );

  var wrongPassphraseRejected = false;
  try {
    final wrongKey = await kdf.deriveKeyFromPassword(
      password: wrongPassphrase,
      nonce: recoverySalt,
    );
    await cipher.decrypt(recoveryBox, secretKey: wrongKey);
  ...
  print(
    'PASS archive_contains_plaintext_database_key=${archive.contains(base64Encode(databaseKey))}',
  );
}
