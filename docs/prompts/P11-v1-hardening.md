...
1. Finish automatic/manual encrypted backups, manifest/attachment consistency, retention, overdue startup handling, disk-space checks and external-drive export. Show last verified backup, not merely last attempted job.
2. Implement staged restore with maintenance mode, safety backup, schema/integrity/hash validation, atomic switch/rollback and post-restore numbering reconciliation. Rehearse replacement-machine recovery with portable keys.
