# P05 — Build inventory, valuation and serial control

Release: V1. Prerequisites: P04.
...
```text
Implement P05: Build inventory, valuation and serial control for Solar Shop ERP.
...
1. Implement stock locations, append-only movements, transactionally maintained balance projections, ledger rebuild and fixed-point quantities. Support opening stock, approved adjustments, reason/approval records and same-branch transfers.
2. Implement serial and batch records, serial state transitions, whole-unit constraints, quarantine and reservation primitives. Availability excludes quarantine/transit and active reservations.
3. Implement per-product/location weighted-average valuation, final-quantity rounding, cost snapshots and variance handling interfaces. Tie stock documents to balanced journal effects.
4. Create stock dashboards, movement/history views and serial lookup. Do not allow UI or data imports to write product stock totals directly.

Expected artifacts: Inventory domain/UI and migrations; docs/implementation/inventory-valuation.md.
Acceptance checks: T02 at inventory command level, T07 and T15 opening import pass; concurrent duplicate serial issue rejects; ledger rebuild equals projections; negative stock blocked; transfer and rollback preserve quantity/value.
Exit gate: Rebuildable stock ledger with consistent serial ownership and valuation.
