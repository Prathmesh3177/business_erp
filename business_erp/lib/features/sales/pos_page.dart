  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: const Color(0xFF8B0000), // Deep Red / Crimson accent
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _HeldDraftsTab(
            onResumeDraft: () => _tabController.animateTo(0),
          ),
...
class _PosCartItem {
  _PosCartItem({
    ...
    this.gstRate = 18.0,
        ...
        priceController = TextEditingController(text: unitPriceRupees.toString()),
...
class _CounterPosTabState extends ConsumerState<_CounterPosTab> {
  ...
  Future<void> _holdDraft() async {
    ...
    try {
      ...
      final draftLinesJson = jsonEncode(_cart.map((c) => {
  ...
  void _showTenderModal() {
    if (_cart.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please add items to cart before proceeding to checkout.')),
      );
      return;
    }

    final cashController = TextEditingController(text: _grandTotalRupees.toStringAsFixed(2));
  ...
  Future<void> _completeCheckout({
    ...
    try {
      ...
      if (cashPaise > 0) tenderLines.add(TenderLine(method: TenderMethod.cash, amountPaise: Money.fromPaise(cashPaise)));
      if (upiPaise > 0) tenderLines.add(TenderLine(method: TenderMethod.upi, amountPaise: Money.fromPaise(upiPaise)));
      if (cardPaise > 0) tenderLines.add(TenderLine(method: TenderMethod.card, amountPaise: Money.fromPaise(cardPaise)));
      if (bankPaise > 0) tenderLines.add(TenderLine(method: TenderMethod.bankTransfer, amountPaise: Money.fromPaise(bankPaise)));
      if (creditPaise > 0) tenderLines.add(TenderLine(method: TenderMethod.customerCredit, amountPaise: Money.fromPaise(creditPaise)));
                  ...
                  children: [
                    Expanded(
                      ...
                      child: DropdownButtonFormField<Party?>(
                        ...
                        items: [
                          ...
                          ..._customers.map((c) => DropdownMenuItem<Party?>(
                                value: c,
                                child: Text('${c.name} (Credit: ₹${c.creditLimitPaise / 100})'),
                              )),
                        ],
                    ...
                    Expanded(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(value: 'MAIN_WH', child: Text('Main WH')),
                    ...
                    child: Column(
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            const Text('Discount:'),
                            Text('- ₹${_discountTotalRupees.toStringAsFixed(2)}'),
                          ],
                        ),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            const Text('GST Tax:'),
                            Text('+ ₹${_taxTotalRupees.toStringAsFixed(2)}'),
                          ],
                        ),
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Held POS Draft Sales',
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ElevatedButton.icon(
                          ...
                          trailing: Column(
                            ...
                            children: [
                              Chip(
                                ...
                                backgroundColor: item.status == SaleStatus.posted
                                    ? Colors.green[700]
                                    : Colors.grey,
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Business Date: ${header.businessDate.toLocal().toString().split(' ')[0]}'),
              Text('Subtotal: ₹${header.subtotalPaise.inRupees.toStringAsFixed(2)}'),
              Text('GST Tax: ₹${header.totalTaxPaise.inRupees.toStringAsFixed(2)}'),
              const Divider(height: 24),
              const Text('Sold Items & Serials:', style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Expanded(
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Grand Total: ₹${header.grandTotalPaise.inRupees.toStringAsFixed(2)}',
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  Row(
                    children: [
                      ElevatedButton.icon(
