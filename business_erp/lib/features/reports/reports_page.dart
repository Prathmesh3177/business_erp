...
final class _ReportsPageState extends ConsumerState<ReportsPage>
  ...
  Future<void> _loadReportData() async {
    ...
    try {
      ...
      final dateRange = ReportDateRange(
        fromDateInclusiveUtc: DateTime.utc(
          _fromDate.year,
          _fromDate.month,
          _fromDate.day,
        ),
        toDateInclusiveUtc: DateTime.utc(
          _toDate.year,
          _toDate.month,
          _toDate.day,
          23,
  ...
  void _exportCsv(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Exported $reportName CSV successfully (${csvData.length} bytes, formula sanitized)',
        ),
      ...
      child: LayoutBuilder(
        builder: (context, constraints) => Wrap(
          ...
          children: [
            ...
            Text(
              'Period: ${_fromDate.toString().split(' ')[0]} to ${_toDate.toString().split(' ')[0]}',
            ),
            TextButton.icon(
  ...
  Widget _buildSalesTab() {
    ...
    if (summary == null || summary.items.isEmpty) {
      return const Center(
        child: Text('No sales records found for selected period.'),
      );
    }
    ...
    final rows = summary.items.map((item) {
      return [
        ...
        if (hasCostData)
          '₹${(item.cogs?.inRupees ?? 0).toStringAsFixed(2)}'
        else
          'Masked',
        if (hasCostData)
          '₹${(item.grossProfit?.inRupees ?? 0).toStringAsFixed(2)}'
        else
          'Masked',
        if (hasCostData)
            ...
            rows: summary.items.map((item) {
              return DataRow(
                cells: [
                  ...
                  DataCell(
                    Text('₹${item.netSales.inRupees.toStringAsFixed(2)}'),
                  ),
                  DataCell(
                    Text(
                      hasCostData
                          ? '₹${item.cogs!.inRupees.toStringAsFixed(2)}'
                          : '***',
                    ),
                  ),
                  DataCell(
                    Text(
                      hasCostData
                          ? '₹${item.grossProfit!.inRupees.toStringAsFixed(2)}'
                          : '***',
                    ),
                  ),
                  DataCell(
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text(
                    'GSTR-1 Outward Taxable Supplies',
                    style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                  ),
                  const Divider(),
                  Text(
                    'B2B Taxable Value: ₹${gstr1?.b2bTaxableValue.inRupees.toStringAsFixed(2) ?? "0.00"}',
                  ),
                  Text(
                    'B2B CGST: ₹${gstr1?.b2bCgst.inRupees.toStringAsFixed(2) ?? "0.00"} | SGST: ₹${gstr1?.b2bSgst.inRupees.toStringAsFixed(2) ?? "0.00"}',
                  ),
                  const SizedBox(height: 8),
                  Text(
                    'B2C Small Taxable Value: ₹${gstr1?.b2cSmallTaxableValue.inRupees.toStringAsFixed(2) ?? "0.00"}',
                  ),
                  Text(
                    'B2C Small CGST: ₹${gstr1?.b2cSmallCgst.inRupees.toStringAsFixed(2) ?? "0.00"} | SGST: ₹${gstr1?.b2cSmallSgst.inRupees.toStringAsFixed(2) ?? "0.00"}',
                  ),
                ],
              ),
            ),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text(
                    'GSTR-3B Summary & Tax Payable',
                    style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                  ),
                  const Divider(),
                  Text(
                    'Outward Taxable Supplies: ₹${gstr3b?.outwardTaxableSupplies.inRupees.toStringAsFixed(2) ?? "0.00"}',
                  ),
                  Text(
                    'Inward ITC Taxable Supplies: ₹${gstr3b?.inwardItcTaxableSupplies.inRupees.toStringAsFixed(2) ?? "0.00"}',
                  ),
                  const SizedBox(height: 8),
                  Text(
                    'Net CGST Payable: ₹${gstr3b?.netCgstPayable.inRupees.toStringAsFixed(2) ?? "0.00"}',
                    style: const TextStyle(
                      fontWeight: FontWeight.bold,
                  ...
                  Text(
                    'Net SGST Payable: ₹${gstr3b?.netSgstPayable.inRupees.toStringAsFixed(2) ?? "0.00"}',
                    style: const TextStyle(
                      fontWeight: FontWeight.bold,
                      color: Colors.deepOrange,
                    ),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                'Stock Valuation Report (${items.length} products)',
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
                ),
                  ...
                  final rows = items
                      .map(
                        (i) => [
                          ...
                          i.unitCost != null
                              ? '₹${i.unitCost!.inRupees.toStringAsFixed(2)}'
                              : 'Masked',
                          i.totalValuationCost != null
                              ? '₹${i.totalValuationCost!.inRupees.toStringAsFixed(2)}'
                              : 'Masked',
                        ],
                      )
                      .toList();
                  _exportCsv('Stock Valuation', [
                    'SKU',
                    'Product',
                    'Category',
                    'On Hand',
  ...
  Widget _buildAgeingTab() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'Customer Accounts Receivable Ageing',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
          ),
          const SizedBox(height: 12),
          ErpResponsiveTable(
                ...
                .map(
                  (a) => DataRow(
                    cells: [
                      ...
                      DataCell(
                        Text('₹${a.currentAmount.inRupees.toStringAsFixed(2)}'),
                      ),
                      DataCell(
                        Text('₹${a.days1To30.inRupees.toStringAsFixed(2)}'),
                      ),
                      DataCell(
                        Text('₹${a.days31To60.inRupees.toStringAsFixed(2)}'),
                      ),
                      DataCell(
                        Text('₹${a.daysOver90.inRupees.toStringAsFixed(2)}'),
                      ),
                      DataCell(
                        Text(
                          '₹${a.totalOutstanding.inRupees.toStringAsFixed(2)}',
      ...
      child: Card(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                'Financial Trial Balance & Profit Statement',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
              SizedBox(height: 8),
