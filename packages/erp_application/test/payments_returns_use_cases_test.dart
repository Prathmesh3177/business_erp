...
class InMemoryFinanceStore implements FinanceStore {
  ...
  ) async => purchaseReturnLines[purchaseReturnId] ?? [];

  @override
  Future<List<PurchaseReturnHeader>> listPurchaseReturns(
    String organizationId,
  ) async => purchaseReturnHeaders.values
      .where((h) => h.organizationId == organizationId)
      .toList();
  ...
  Future<List<ExpenseCategory>> getExpenseCategories(
    String organizationId,
  ) async => expenseCategories.values.toList();
...
class InMemoryInventoryStore implements InventoryStore {
  ...
  Future<StockBalance?> getStockBalance(
    String productId,
    String locationId,
  ) async => balances['${productId}_$locationId'];
  @override
  ...
  Future<SerialRecord?> getSerialByNumber(
    ...
    String productId,
    String serialNumber,
  ) async => serials[serialNumber];
  @override
  Future<List<SerialRecord>> getSerialsForProduct(
    String organizationId,
    String productId, {
    SerialState? state,
  }) async => serials.values.toList();
  @override
...
class InMemoryAccountingStore implements AccountingStore {
  ...
  }) async => '$series/2627/${(seq++).toString().padLeft(6, '0')}';
  @override
...
void main() {
  ...
  group('P08 Payments, Returns & Expenses Use Cases', () {
    test('Posts sales return, reverses COGS, returns serial to stock, and balances Credit Note journal', () async {
      ...
      final sumDebit = crnJournal.lines.fold<int>(
        0,
        (sum, l) => sum + l.debitPaise,
      );
      final sumCredit = crnJournal.lines.fold<int>(
        0,
    ...
    test(
      'Rejects return if cumulative quantity exceeds original line quantity',
      () async {
        final context = CommandContext(
          session: adminSession,
          timestampUtc: DateTime.now(),
    ...
    test('Posts expense entry and balanced double-entry journal', () async {
      ...
      expect(
        accountingStore.journals.first.lines.first.debitPaise,
        equals(1500000),
      );
    });

    test(
      'Closes cash session and posts variance journal on cash shortage',
      () async {
        final context = CommandContext(
          session: adminSession,
          timestampUtc: DateTime.now(),
        ...
        expect(accountingStore.journals.length, equals(1));
        expect(
          accountingStore.journals.first.lines.first.accountId,
          equals('acc_6900'),
        );
      },
    );
